...
/***/ function(module, exports, __webpack_require__) {
	...
	var PredictionEngineErrorMessage = 'PredictionEngine requires getPredictor, updateState, and getStateDescription functions';

	function getCompetitorPredictors(PredictionEngine, config, state) {
	  var numPlayers = Math.random() * (config.max_players - config.min_players) + config.min_players;
	  var players = [];
	...
	function wrapPredictorWithId(predictor) {
	  return _extends({}, predictor, { __id__: _uuid2['default'].v4() });
	}

	function runGame(testPredictor, competitorPredictors) {
	  var players = [testPredictor].concat(_toConsumableArray(competitorPredictors));
	  var result = _game2['default'].play(players);
	  return result.list.find(function (player) {
	    return player.__id__ === testPredictor.__id__;
	  }).score;
	}

	function verifyPredictionEngine(PredictionEngine) {
	  if (typeof PredictionEngine !== 'object' || typeof PredictionEngine.getInitialState !== 'function' || typeof PredictionEngine.shouldStop !== 'function' || typeof PredictionEngine.getPredictor !== 'function' || typeof PredictionEngine.updateState !== 'function' || typeof PredictionEngine.getStateDescription !== 'function') {
	...
	function learn(PredictionEngine) {
	  var config = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];

	  verifyPredictionEngine(PredictionEngine);
	  verifyConfig(config);

	  var state = PredictionEngine.getInitialState();
	  var results = {};
	  ...
	  while (!PredictionEngine.shouldStop(state)) {
	    ...
	    var sumScore = 0;
	    var runsToSum = 300;
	    for (var i = 0; i < runsToSum; i++) {
	      var result = runGame(wrapPredictorWithId(PredictionEngine.getPredictor(state)), getCompetitorPredictors(PredictionEngine, config, state), config);
	      sumScore += result;
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _noThanksEngine = __webpack_require__(3);

	var _noThanksEngine2 = _interopRequireDefault(_noThanksEngine);

	exports['default'] = {
	  play: function play(players) {
	    ...
	    var playerIds = players.map(function (_ref) {
	      var __id__ = _ref.__id__;
	    ...
	    while (gameState.game.ongoing) {
	      var currentPlayerId = gameState.players.list[gameState.players.currentPlayer].__id__;
	      var legalActions = _noThanksEngine2['default'].getLegalActions(gameState);
	      var action = playerMap[currentPlayerId].predict(gameState, legalActions);
	      gameState = _noThanksEngine2['default'].resolveAction(gameState, action);
	    }
